...
static double measure_cpu_performance(MPI_Comm Communicator)
{
  ...
  mpi_printf(
      ...
      max_timenode.rank, timediff(ta, tb));

  // if(variation > MAX_VARIATION_TOLERANCE)
  //   terminate("We are stopping because the performance variation=%g of the CPUs lies above the prescribed tolerance
