  ...
  for(int idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      double fac  = 0.5 * dmax(sqrt(cf * cf + vpot * vpot), 0.01 * All.DednerSpeed) / get_cell_radius(i);
...
#ifdef ENHANCED_DEDNER_VARIABLE_SPEED
      SphP[i].DednerSpeed = 5*get_sound_speed(i);
#endif
...
#ifdef MHD_DEDNER_SPEED_BOOST_FACTOR
	All.DednerSpeed *= MHD_DEDNER_SPEED_BOOST_FACTOR;
#endif
