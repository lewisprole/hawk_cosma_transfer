      ...
      if(self->allsky)
        {
          ...
          maxrad        = dmin(asin(SphP[igas].MaxDelaunayRadius / radius), 2. * M_PI);
      ...
      if(self->allsky)
        {
          ...
          hsml       = dmax(hsml, 1.001 * dmax(dx, dy) * 0.5);
        ...
        {
          hsml = dmax(P[idm].Auriga_Movie_Hsml, 1.001 * dmax(dx, dy) * 0.5);
      ...
      if(self->allsky)
        {
          ...
          hsml       = dmin(hsml, dmax(4. * dmax(dx, dy), 2. * M_PI / 720.));
          hsml       = dmax(hsml, 1.001 * dmax(dx, dy) * 0.5);
        ...
        {
          hsml = dmin(P[istar].Auriga_Movie_Hsml, dmax(4. * dmax(dx, dy), 2e-5 / All.Time * All.HubbleParam));
          hsml = dmax(hsml, 1.001 * dmax(dx, dy) * 0.5);
          if(px + hsml < -0.5 * self->width || px - hsml > +0.5 * self->width || py + hsml < -0.5 * self->height ||
             py - hsml > +0.5 * self->height || pz < -0.5 * self->depth || pz > +0.5 * self->depth)
            continue;
