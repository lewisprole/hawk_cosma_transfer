  ...
  for(i = 0; i < Nstar; i++)
    {
      ...
      dt = (P[StarParticle[i].index].TimeBinGrav ? (((integertime)1) << P[StarParticle[i].index].TimeBinGrav) : 0) *
      ...
      age_of_star_in_Gyr_endstep = get_time_difference_in_Gyr(
          STP(StarParticle[i].index).BirthTime,
          time_begstep); /* (Note: All.Ti_begstep[] has already been advanced for the next step at this point) */
      age_of_star_in_Gyr = age_of_star_in_Gyr_endstep - dtime_in_Gyr;

#ifdef GFM_RPROCESS_CHANNELS_NS_KICKS
      /* check if this is a NSNS particle, and if yes only check if it does r-process enrichment */
#endif
      ...
      //STP(StarParticle[i].index).NumSNIa += sed.NumSNIa; // don't add the number here but in enrich and only for the Ia's that actually injected energy
#endif
...
#else
      StarParticle[i].TotalEnergyReleased =
          compute_total_SN_energy(StarParticle[i].index, age_of_star_in_Gyr + dtime_in_Gyr, &SN_Number);
#endif
      ...
      double tol                  = 1.0e-5;
      StarParticle[i].BlastRadius = compute_blast_radius(StarParticle[i].TotalEnergyReleased, StarParticle[i].AvgHDens * All.cf_a3inv,
                                                         StarParticle[i].AvgPress * All.cf_a3inv) /
                                    All.cf_atime;
      ...
      StarParticle[i].CoolShutoffTime = compute_cooling_shutoff_time(
          StarParticle[i].TotalEnergyReleased, StarParticle[i].AvgHDens * All.cf_a3inv, StarParticle[i].AvgPress * All.cf_a3inv);
      ...
      if(age_of_star_in_Gyr >= All.InputTimeHeatRadiationFeedback)
      ...
      else
        {
          ...
          if(age_of_star_in_Gyr >= All.InputTimeMomRadiationFeedback && STP(StarParticle[i].index).RadFeed_Flag == 0)
            {
              StarParticle[i].RadiationMomentumReleased =
                  Lum / CLIGHT * age_of_star_in_Gyr * SEC_PER_GIGAYEAR; /* gr (cm/s) h^-1 ... */
              StarParticle[i].RadiationMomentumReleased /=
          ...
          StarParticle[i].RadCoolShutoffTime = (All.InputTimeHeatRadiationFeedback - age_of_star_in_Gyr) * SEC_PER_GIGAYEAR *
          ...
          StarParticle[i].RadCoolShutoffTime =
              dtime_in_Gyr * SEC_PER_GIGAYEAR * All.HubbleParam / All.UnitTime_in_s; /* in code units */
#endif
        }
      ...
      if(age_of_star_in_Gyr > 0.004) /*After 4 Myr no more input */
        ...
        {
          StarParticle[i].EarlyTotalEnergyReleased = compute_EarlyFeedback_energy(age_of_star_in_Gyr, P[StarParticle[i].index].Mass,
                                                                                  dtime); /* dtime in sec, returns in code units  */
          // printf("FM_EARLY ENERGY=, age=%g, dt=%g, Energy=%g \n",age_of_star_in_Gyr, dtime,
          // StarParticle[i].EarlyTotalEnergyReleased/(All.HubbleParam / All.UnitEnergy_in_cgs));  /* output energy in cgs, time in sec
          // */
        }
