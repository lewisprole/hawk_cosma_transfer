...
#ifdef FM_STOCHASTIC_HII_PHOTOIONIZATION
  in->Velocity                 = compute_radiation_feedback_velocity(i, StarParticle[i].NormSphRadFeedback,
                                                     STP(StarParticle[i].index).Hsml * All.cf_atime, RadiationMomentumReleased);
...
#else
  in->Velocity = compute_radiation_feedback_velocity(i, StarParticle[i].NormSphRadFeedback,
                                                     StarParticle[i].StromgrenRadius * All.cf_atime, RadiationMomentumReleased);
#endif
...
int radiation_feedback_evaluate(int target, int mode, int thread_id)
{
  ...
  h   = in->Hsml;
  if(h > 0.1)
    h = 0.1;  // ToDo:  At large distances the ionizing photon input rate needs to account for the geometric incidence of radiation
  ...
  if(in->NormSphRadFeedback <= 0 && h > 0)
    terminate(
        "tot_mass<=0 should not happen in radiation_stellar_feedback mode 0: pos %g|%g|%g, tot_mass %g, NumNgb %g, h %g , "
        "RadiationMomentumReleased %g, velocity %g,  ID = %d\n",
        pos[0], pos[1], pos[2], tot_mass, StarParticle[target].NumNgb, h, RadiationMomentumReleased, velocity,
        P[StarParticle[target].index].ID);
          ...
          if(r2 < h2)
            {
              ...
              rho   = SphP[j].Density * All.cf_a3inv * All.UnitDensity_in_cgs * All.HubbleParam *
                    All.HubbleParam; /* convert to physical cgs units */
              ne   = SphP[j].Ne;
              temp = convert_u_to_temp(uphys, rho, &ne);

              /* identify gas that could be photoionized, but make sure we can "reselect" gas that has just returned to
               * non-photoionized state */
...
void do_radiation_stellar_feedback(void)
{
  ...
  mpi_printf(
      "FM_RADIATION_FEEDBACK:  ---> number of kicked cells = %d, active stars (with SN explosions) = %d, kicked cells / star = %g\n",
  ...
  mpi_printf(
      "FM_RADIATION_FEEDBACK:  ---> as computed in SphP struct  min shutoff time (int units)= %g, max shutoff time (int units)= %g\n",
          ...
          if(StarParticle[i].RadiationMomentumReleased > 0)
            {
              ...
              momentum_diff = sqrt(StarParticle[i].deltaMomentum[0] * StarParticle[i].deltaMomentum[0] +
                                   StarParticle[i].deltaMomentum[1] * StarParticle[i].deltaMomentum[1] +
                                   StarParticle[i].deltaMomentum[2] * StarParticle[i].deltaMomentum[2]);
            }
          else
            {
          ...
          if(r2 < h2)
            {
              ...
              /// yhelium = (1 - SphP[j].MetalsFraction[element_index_Hydrogen] - SphP[j].Metallicity) / (4. *
              /// SphP[j].MetalsFraction[element_index_Hydrogen]); meanweight = (1 + 4 * yhelium) / (1 + yhelium + SphP[j].Ne);
              u = SphP[j].Utherm;
              uphys = u * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs;
              rho = SphP[j].Density * All.cf_a3inv * All.UnitDensity_in_cgs * All.HubbleParam *
                    All.HubbleParam; /* convert to physical cgs units */
                                             ...
                                             /*
                                                                   ...
                                                                   SphP[j].Utherm =
                                                                     (SphP[j].Energy -
                                                                             ...
                                                                             SphP[j].Momentum[2] * SphP[j].Momentum[2]) / P[j].Mass) / P[j].Mass / (All.cf_atime *
                                             All.cf_atime);                                SphP[j].A = (GAMMA - 1.0) * SphP[j].Utherm / pow(SphP[j].Density *
                                             All.cf_a3inv, GAMMA - 1);                                SphP[j].Entropy = log(SphP[j].A) * P[j].Mass;                                #endif
                                             */
            }                                /* if r2<rs2) */
...
MyFloat compute_radiation_feedback_velocity(int n, MyFloat stromgren_mass, MyFloat stromgren_radius, MyFloat radiatmom)
{
  ...
  if(rs_phy > 0)
    sigma_gas = mass_phy / (3.14159 * rs_phy * rs_phy);
  else
    sigma_gas = 0;
