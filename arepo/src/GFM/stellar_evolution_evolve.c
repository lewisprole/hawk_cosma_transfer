...
MyFloat get_NSNS_delay_time_single_event(int idx, MyFloat time_to_redshift_zero_in_Gyr)
{
  ...
  if(fabs(All.rp[idx].NSNS_PowerlawIndex - 1.) < 1e-6)
    {
      return exp(r / All.rp[idx].NSNS_RateNorm + log(time_to_redshift_zero_in_Gyr - All.rp[idx].NSNS_RateTAU)) +
             All.rp[idx].NSNS_RateTAU;
    }
  else
    {
      return exp(log(r / (All.rp[idx].NSNS_RateNorm * (1. - All.rp[idx].NSNS_PowerlawIndex)) + time_to_redshift_zero_in_Gyr -
                     All.rp[idx].NSNS_RateTAU) /
                 (1. - All.rp[idx].NSNS_PowerlawIndex)) +
             All.rp[idx].NSNS_RateTAU;
    }
}
  ...
  for(idx = 0; idx < GFM_RPROCESS_NSNS; idx++)
    {
      ...
      if(age_of_star_in_Gyr > All.rp[idx].NSNS_RateTAU)
        {
          ratebegin = All.rp[idx].NSNS_RateNorm * pow(age_of_star_in_Gyr - All.rp[idx].NSNS_RateTAU, -All.rp[idx].NSNS_PowerlawIndex);
        }
      double rateend = 0.;
      if(age_of_star_in_Gyr + dt_in_Gyr > All.rp[idx].NSNS_RateTAU)
        {
          rateend = All.rp[idx].NSNS_RateNorm *
                    pow(age_of_star_in_Gyr + dt_in_Gyr - All.rp[idx].NSNS_RateTAU, -All.rp[idx].NSNS_PowerlawIndex);
        }

      // subcycle to get a better sampling of the rate
      int count = imax((int)((ratebegin - rateend) / rateend / 0.001) + 1, 10);
      for(i = 0; i < count; i++)
        {
          MyFloat NSNS_number = get_NSNS_number_dtd_powerlaw(idx, age_of_star_in_Gyr + (dt_in_Gyr / count) * i, dt_in_Gyr / count);
