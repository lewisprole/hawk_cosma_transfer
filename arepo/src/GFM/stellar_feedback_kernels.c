              ...
              if(TotalEnergyReleased > 0)
                {
                  ...
                  if(h > cool_rad)
                    {
                      ...
                      du = egy_at_cool_rad * (cool_rad * cool_rad * cool_rad * cool_rad * cool_rad * cool_rad) /
                           (h * h * h * h * h * h) * weight_fac / P[j].Mass;
                    }
                  else
                    {
                  ...
                  for(k = 0; k < 3; k++)
                    SphP[j].Energy -= 0.5 * SphP[j].Momentum[k] * SphP[j].Momentum[k] / P[j].Mass;
...
void stochastic_momentum_feedback(int target, int mode, int thread_id, int numnodes, int *firstnode, data_in *in, data_out *out)
{
  ...
  if(prob > 1.0)
    terminate("Total mass within kernel is less than mass to be kicked for stochastic feedback: tot_mass %g, kick_mass %g", tot_mass,
              prob * tot_mass);
              ...
              if(TotalEnergyReleased > 0)
                {
                  if(prob > 1.0)
                    terminate(
                        "Total mass within kernel is less than mass to be kicked for stochastic feedback: tot_mass %g, kick_mass %g",
