...
void open_logfiles(void)
{
  ...
  if(ThisTask == 0)
    {
      sprintf(buf, "%ssinks_mergers", All.OutputDir);
      mkdir(buf, 02755);
    }
  MPI_Barrier(MPI_COMM_WORLD);
  /* Note: This is done by everyone */
  sprintf(buf, "%ssinks_mergers/sinks_mergers_%d.txt", All.OutputDir, ThisTask);
...
#ifdef SIMPLEX
  sprintf(buf, "%s%s", All.OutputDir, "simplex.txt");
...
#ifdef SN_MCS
  sprintf(buf, "%s%s", All.OutputDir, "snr.txt");
  if(!(FdSnr = fopen(buf, mode)))
    terminate("error in opening file '%s'\n", buf);
#endif

#ifdef SN_MCS_LOG
  sprintf(buf, "%s%s", All.OutputDir, "sndens.txt");
  if(!(FdSNdens = fopen(buf, mode)))
    terminate("error in opening file '%s'\n", buf);
#endif

#ifdef SFR_MCS_LOG
  sprintf(buf, "%s%s", All.OutputDir, "sfdens.txt");
  ...
  if(open_sink_particle_evolution_file() == 0)
    {
      sprintf(buf, "error in opening sink file... Too many already?");
      terminate(buf);
    }
#endif
#endif
  ...
  sprintf(buf, "%s%s", All.OutputDir, "sgs_turbulence.txt");
  if(!(FdSgsTurbulence = fopen(buf, mode)))
    terminate("error in opening file '%s'\n", buf);
#if defined(SGS_TURBULENCE_LOG_PRODUCTION_DISSIPATION)
  sprintf(buf, "%s%s", All.OutputDir, "sgs_turbulence_production_dissipation.txt");
