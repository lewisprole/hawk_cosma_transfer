...
void compute_grav_accelerations(int timebin, int fullflag)
{
  ...
  if(All.Ti_Current == 0 && RestartFlag == RESTART_IC)
...
#if defined(PMGRID) && defined(PERIODIC) && !defined(GRAVITY_NOT_PERIODIC) && !defined(ONLY_PM)
...
#endif /* #if defined(PMGRID) && defined(PERIODIC) && !defined(GRAVITY_NOT_PERIODIC) && !defined(ONLY_PM) */
