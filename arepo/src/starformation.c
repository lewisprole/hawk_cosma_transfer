...
#if !defined(FM_SFR) && !defined(SFR_MCS)
  init_clouds();
#else
  init_star_formation();
          ...
          if(SphP[i].Sfr > 0)
            {
              ...
              if(P[i].Mass < 2.0 * All.TargetGasMass)
                ...
                mass_of_star = dmin(P[i].Mass, All.StellarMassToGasMassRatio * All.TargetGasMass);
#else
...
#ifdef SF_STELLAR_MASS_TO_GAS_MASS_RATIO
                mass_of_star = All.StellarMassToGasMassRatio * All.TargetGasMass;
#else
  ...
  if(get_random_number() < prob)
    {
      ...
      for(int ich = 0; ich < GFM_RPROCESS_NSNS; ich++)
        {
          double dtime = get_time_difference_in_Gyr(All.Time, 0);
          ...
          for(int iNSNS = 0; iNSNS < NSNS_number_true; iNSNS++)
            {
              ...
              P[j]      = P[idx_of_star];
              P[j].Mass = 0;
              timebin_add_particle(&TimeBinsGravity, j, idx_of_star, P[j].TimeBinGrav, TimeBinSynchronized[P[j].TimeBinGrav]);
#if defined(TRACER_MC)
              P[j].TracerHead      = -1; /* new star particle starts with no tracer children */
              ...
              for(int k = 0; k < 3; k++)
                P[j].Vel[k] += (All.rp[idx].NSNS_KickAverage + gsl_ran_gaussian(random_generator, All.rp[idx].NSNS_KickSigma)) *
                               All.cf_atime / All.UnitVelocity_in_cm_per_s * 1e5; /* constants in km/s */
