...
static void parallel_merge(int master, int ncpu, size_t *nlist, size_t nmax, char *base, size_t nmemb, size_t size,
...
void parallel_merge(int master, int ncpu, size_t *nlist, size_t nmax, char *base, size_t nmemb, size_t size,
                    int (*compar)(const void *, const void *))
{
  size_t na, nb, nr;
          ...
          while(na >= nlist[cpua] && cpua < master + ncpu / 2 - 1)
            {
              cpua++;
              if(nlist[cpua])
                MPI_Recv(list_a, nlist[cpua] * size, MPI_BYTE, cpua, TAG_DATAIN, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
              na = 0;
            }
