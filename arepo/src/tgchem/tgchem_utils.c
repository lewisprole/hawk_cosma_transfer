...
void tgchem_cell_data(step_data *pstep, cell_data *pcell, int idx)
{
  ...
  double lsob      = ((divv == 0.) ? ljeans : dmin(vth / dabs(divv), ljeans));
...
void tgchem_dep_vars(double abh2, double abhii, double nh, double *abhi, double *abe, double *mu, double *ntot)
{
  ...
  *abe  = abhii;
  *mu   = (1. + 4. * HE_ABUND) / dmax(1. + HE_ABUND - abh2 + abhii, 0.);
  *ntot = dmax(1. + HE_ABUND - abh2 + *abe, 0.) * nh;
}
...
void tgchem_gamma_temp(double abh2, double abe, double energy, double ntot, double *gamma, double *temp)
{
  ...
  double abn = dmax(1. + HE_ABUND - abh2 + abe, 0.);
  double ab  = dmax(1. + HE_ABUND - 2 * abh2 + abe, 0.);
...
{
  abhm = dmin(dmax(abhm, 0.), TGCD.AbMax[0]);
...
{
  abh2 = dmin(dmax(abh2, 0.), TGCD.AbMax[1]);
...
{
  abhii = dmin(dmax(abhii, 0.), TGCD.AbMax[2]);
...
{
  temp = dmin(dmax(temp, TGCHEM_TEMP_MIN), TGCHEM_TEMP_MAX);
...
{
  h2_column = dmin(dmax(h2_column, TGCHEM_H2_COLUMN_MIN), TGCHEM_H2_COLUMN_MAX);
...
double tgchem_t_ff(double nh)
{
  ...
  rho = PROTONMASS * nh / HYDROGEN_MASSFRAC;
