...
void tgchem_rates_aux(cell_data* pcell, double dt_step)
{
  ...
  for(i = 0; i < TGCHEM_NUM_SPECIES; i++)
    dt_rate[i] = species[i] / dabs(tot_rate[i]);
  ...
  double dt_chem = species[TGCHEM_NUM_ABUNDANCES] / dabs(tot_chem_rate);
  ...
  double dt_rad = species[TGCHEM_NUM_ABUNDANCES] / dabs(tot_rad_rate);
          ...
          if(nh > 1e10)
            {
              ...
              for(j = 0; j < TGCHEM_NUM_CHEM_RATES; j++)
                {
                  max_prate = dmax(max_prate, prate[i][j]);
                  max_nrate = dmax(max_nrate, nrate[i][j]);
                }
...
double tgchem_h2_line_fesc(double nh, double temp, double h2_column)
{
  ...
  if(TGCD.ChemH2Mode == 0)
  ...
  else if(TGCD.ChemH2Mode == 1)
    {
      ...
      int idx2 = dmin((temp_idx + 1), TGCHEM_NUM_TEMP - 1) * TGCHEM_H2_NUM_COLUMN + h2_column_idx;
      ...
      fesc = dmin(sob_emiss / tgchem_cool_coeff(1, temp), 1.);
    }
...
double tgchem_h2_cie_fesc(double nh)
{
  ...
  double tau = dmax(pow(x, 2.8), 1e-10);
...
double tgchem_hm_fesc(double nh, double temp, double ljeans)
{
  ...
  double tau = dmax(kappa * rho * ljeans, 1e-10);
