  ...
  if(dt_hydro)
    {
      ...
      while(!flag_break)
        {
          dt_step = dmax(dt_hydro - dt_sum, 0.);
          ...
          if(eq_h2_flag)
            {
              if(!eq_hii_flag)
                dt_step = dmin(TGCHEM_TOL_EQ_STEPSIZE * dt_hii, dt_step);

              dt_step = dmin(TGCHEM_TOL_EQ_STEPSIZE * dt_rad, dt_step);
            }
          else
            {
              dt_step = dmin(dt_h2, dt_step);
              dt_step = dmin(dt_hii, dt_step);
              dt_step = dmin(dt_rad, dt_step);
            }
  ...
  while(energy_new != energy_min && energy_new != energy_max)
    {
      ...
      if(dabs(energy_new - energy_old) < TGCHEM_TOL * energy_old)
        break;
    }
...
double tgchem_bisection_hii(double abh2, double abhii, double energy, double nh)
{
  ...
  abhii_max = dmin(abhii_max, TGCD.AbMax[2]);
  ...
  while(abhii_new != abhii_min && abhii_new != abhii_max)
    {
      ...
      if(dabs(abhii_new - abhii_old) < TGCHEM_TOL * abhii_old)
        break;
    }
...
void tgchem_finish_step(step_data* pstep, cell_data* pcell)
{
  ...
  Utherm_new        = dmax(Utherm_new, All.MinEgySpec);
