...
void sx_initialize(void)
{
  ...
  mpi_printf("SX-ERROR: SPRAI will not work without a compiler flag DO_NOT_RANDOMIZE_DOMAINCENTER. \n\n");
  endrun();  
#endif
#if (SX_RUNNING_MODE!=2)
  mpi_printf("SX-ERROR: SX_RUNNING_MODE!=2 is currently not available \n\n");
  endrun();
#endif
...
/*!
 * \brief Solve the site for incomming photons and photons from the source
 */
void sx_evolve_sites()
{
#ifdef SX_DISPLAY_TIMERS
...
void sx_evolve_pp()
{
  ...
  if (Argv[4]==0||Argv[5]==0)
    {
      mpi_printf("SX-ERROR: Missing argument <TimeMax> or <numRuns>!! \n\n");
      endrun();
    }
