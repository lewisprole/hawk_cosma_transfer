...
int sxNumSPD;                          // Number of SphP Directions ( NumGas * SX_NDIRS )

struct sxABPP_struct                   // Photon Packages structure
{
...
int sxMaxNumABPP;                      // maximum number of PP in the APP/BPP arrays              
int sxSwapABPP;                        // indication if the ABPP arrays are swapped
int sxNumReallocABPP;                  // counter that counts number of memory reallocations
 
int sxNumAPP, sxNumBPP;                // current number of PP in the APP/BPP arrays
int *sxSPDtoAPP, *sxSPDtoBPP;          // fake connections between SPD and APP/BPP
int *sxRealSPDtoAPP, *sxRealSPDtoBPP;  // real connections between SPD and APP/BPP

//======================================================================================================//
//                                     Photon Packet Exchange variables                                 //
//======================================================================================================//

int sxNumTotEDP;                       // number of all External DP
int sxNumTotEDPD;                      // number of all External DP Directions ( sxNumTotEDP * SX_NDIRS )

struct sxQPP_struct                    // Photons Packets added to the exchange Queue
{
  MySxDouble nPhotons[SX_NFREQ];       // photons for exchange
  int index;                           // SphP index of the particle on a corresponding task
} *sxQPP; 
int sxMaxNumQPP;                       // number of photon packets
int *sxEDPDtoQPP;                      // indexQPP[task][indexEDP][dir] ( sxNumTotEDPD )
int sxNumReallocQPP;                   // counter that counts number of memory reallocations

struct sxEPP_struct                    // Photon Packet Exchange structures
{
  MySxDouble nPhotons[SX_NFREQ];       // photons for exchange
  int index;                           // SphP index of the particle on a corresponding task
  int dir;                             // directional bin
} *sxExportPP;
int sxNumExportPP, sxNumImportPP;      // number of photon packets
int *sxNumEDP, *sxOffEDP;              // numEDP[task] and offsetsEDP[task] (NTask)
int *sxDPtoEDP;                        // indexEDP[indexDP] (Mesh.Ndp)

//======================================================================================================//
//                                     Other stuff variables                                            //
//======================================================================================================//

int SXPID;                             //DEBUG: ID of the debugged particle 
int SXID;                              //DEBUG: current index and process of the sph particle in SphP and P arrays
                                       //       this index can be changed when the particle mash is recalculated!!!

gsl_rng *sxRand;   // random number used during the calculation of a directional vector
...
void sx_phot_sources_start_run();
void sx_phot_sources_end_run();

// Photon Exchange functions
void sx_phot_exchange_start_run();
...
void sx_phot_exchange_photons();
void sx_phot_exchange_end_run();

// Photon transport
void sx_phot_transport_start_run();
void sx_phot_diffuse_transport( struct sxSite *site );
void sx_phot_ballistic_transport( struct sxSite *site );
void sx_phot_transport_copy();
void sx_phot_transport_end_run();
