...
{  
  ...
  if(!(fd = fopen(All.sxTestSrcFile, "r")))
    {
      mpi_printf("SX-ERROR: Could not find the binary file %s \n\n", All.sxTestSrcFile);
      endrun();
    }
  ...
  if (tsNumFreq!=SX_NFREQ)
    {
      mpi_printf("SX-ERROR: Number of frequency bins of a test source %d has to be equal to %d \n\n", tsNumFreq, SX_NFREQ);
      endrun();      
    }
  if (tsNumSigma>0 && tsNumSigma!=SX_NSIGMA)
    {
      mpi_printf("SX-ERROR: Number of cross sections in the test source %d has to be equal to %d \n\n", tsNumSigma, SX_NSIGMA);
      endrun();      
    }
  if (tsNumEnergy>0 && tsNumEnergy!=SX_NENERGY)
    {
      mpi_printf("SX-ERROR: Number of energies in the test source %d has to be equal to %d \n\n", tsNumEnergy, SX_NENERGY);
      endrun();      
    }

  // Read and change crossections if set in the source file
  if (tsNumSigma>0)
    {
