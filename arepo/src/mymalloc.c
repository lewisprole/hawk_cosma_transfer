...
#ifdef GFM
  cc += sprintf(p + cc,
                "-- Skipping: yieldsSNIa, yieldsSNII, yieldsAGB, coolingMetalTab, netCoolingRate, netHeatingRate, netCoolingRateLowZ, "
  ...
  for(unsigned int i = 0; i < Nblocks; i++)
    {
      ...
      if(strncmp(VarName + i * MAXCHARS, "yieldsSNIa", 10) == 0 || strncmp(VarName + i * MAXCHARS, "yieldsSNII", 10) == 0 ||
         strncmp(VarName + i * MAXCHARS, "yieldsAGB", 9) == 0 || strncmp(VarName + i * MAXCHARS, "netCoolingRate", 14) == 0 ||
         strncmp(VarName + i * MAXCHARS, "stellarLuminosityTable", 22) == 0 ||
         strncmp(VarName + i * MAXCHARS, "netHeatingRateLowZ", 18) == 0 ||
         strncmp(VarName + i * MAXCHARS, "netCoolingRateLowZ", 18) == 0 || strncmp(VarName + i * MAXCHARS, "netHeatingRate", 14) == 0)
        {
          continue;
        }
      cc += sprintf(p + cc, "%4d %5u %d %40s  %10.4f   %10.4f  %s%s()|%s|%d\n", ThisTask, i, MovableFlag[i], VarName + i * MAXCHARS,
...
void myfree_movable_fullinfo(void *p, const char *func, const char *file, int line)
{
  ...
  if((unsigned long)nr < Nblocks - 1) /* the block is not the last allocated block */
    {
      /* check that all subsequent blocks are actually movable */
      for(unsigned int i = nr + 1; i < Nblocks; i++)
        if(MovableFlag[i] == 0)
          {
            dump_memory_table();
            myflush(stdout);
  ...
  for(unsigned int i = nr + 1; i < Nblocks; i++)
    {
      ...
      memcpy(VarName + (i - 1) * MAXCHARS, VarName + i * MAXCHARS, MAXCHARS);
      memcpy(FunctionName + (i - 1) * MAXCHARS, FunctionName + i * MAXCHARS, MAXCHARS);
      memcpy(ParentFileName + (i - 1) * MAXCHARS, ParentFileName + i * MAXCHARS, MAXCHARS);
      memcpy(FileName + (i - 1) * MAXCHARS, FileName + i * MAXCHARS, MAXCHARS);
