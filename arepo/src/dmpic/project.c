...
void dmpic_make_image(void)
{
  ...
  construct_forcetree(0, 0, 0, 0); /* build forcetree for all particles */
      ...
      while(1)
        {
          ...
          int count = sscanf(buf, " %d %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg ", &nr, &pictime, &cx, &cy, &cz, &ax, &ay, &az,
                             &bx, &by, &bz, &depth, &angle);
