...
static void io_func_center_of_mass(int particle, int components, void *buffer, int mode)
{
  if(mode == 0)
    {
      MyOutputFloat *pp = (MyOutputFloat *)buffer;
      ...
      for(int k = 0; k < 3; k++)
        {
          pp[k] = SphP[particle].Center[k] - All.GlobalDisplacementVector[k];
...
void init_io_fields()
{
  ...
  init_field(IO_NHE0, "NHeO", "NeutralHeliumAbundance", MEM_NONE, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_NONE, 0, io_func_nHe0,
             GAS_ONLY);                  /* neutral hydrogen fraction */
  init_units(IO_NHE0, 0, 0, 0, 0, 0, 0); /* dimensionless fraction */

  init_field(IO_NHEP, "NHep", "SinglyIonizedHeliumAbundance", MEM_NONE, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_NONE, 0, io_func_nHep,
             GAS_ONLY);                  /* neutral hydrogen fraction */
  init_units(IO_NHEP, 0, 0, 0, 0, 0, 0); /* dimensionless fraction */

  init_field(IO_NHEPP, "Hepp", "DoublyIonizedHeliumAbundance", MEM_NONE, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_NONE, 0, io_func_nHepp,
             GAS_ONLY);                   /* neutral hydrogen fraction */
  init_units(IO_NHEPP, 0, 0, 0, 0, 0, 0); /* dimensionless fraction */
#endif
  ...
  if(All.StarformationOn)
    {
      ...
      init_field(IO_GFM_RPROCESS_COUNT, "NRIA", "GFM_SNIa_Count", MEM_INT, FILE_INT, FILE_NONE, 1, A_STARP,
                &StarP[0].NumSNIa, 0, STARS_ONLY);
#endif
  ...
  init_field(IO_BLAST_RADIUS, "BRD ", "BlastRadius", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_STARP, &StarP[0].BlastRadius, 0,
             STARS_ONLY);
  init_field(IO_HSML_STARS, "SSML", "StarsHsml", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_STARP, &StarP[0].Hsml, 0, STARS_ONLY);
  init_field(IO_COOL_DELAY, "CSO ", "CoolShutoffTime", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_SPHP, &SphP[0].CoolShutoffTime,
             0, GAS_ONLY);
#endif

#ifdef MRT_LOCAL_FEEDBACK
  init_field(IO_BLAST_RADIUS, "MDEN", "MeanDensity", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_NONE, 0, io_func_meandensity,
             STARS_ONLY);
#endif

#ifdef OUTPUT_EARLY_STELLAR_FEEDBACK
  init_field(IO_EARLY_STAR_FEEDBACK_MOMENTUM, "EFMO", "EarlyFeedbackMomentum", MEM_MY_DOUBLE, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_STARP,
             &StarP[0].EarlyCumulativeFeedback, 0, STARS_ONLY);
  init_field(IO_EARLY_STAR_FEEDBACK_CUM_MOMENTUM, "EFCM", "EarlyFeedbackCumMomentum", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1,
             A_STARP, &StarP[0].EarlyFeedbackMomentum, 0, STARS_ONLY);
#endif

#ifdef DELAYED_COOLING_TURB
  init_field(IO_TURBULENT_ENERGY, "UTUR", "TurbulentEnergy", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP,
             &SphP[0].Uturb, 0, GAS_ONLY);
#endif
  ...
  init_field(IO_RADIATION_KICK, "RFVK", "RadVelocityKick", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_STARP,
             &StarP[0].RadVelocityKick, 0, STARS_ONLY);
  ...
  init_field(IO_EARLY_STAR_FEEDBACK, "EFEN", "EarlyCumulativeFeedback", MEM_MY_DOUBLE, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_STARP,
             &StarP[0].EarlyCumulativeFeedback, 0, STARS_ONLY);
#endif
  ...
  init_field(IO_FEEDBACK_EVENTS, "FEVT", "FeedbackEvents", MEM_INT, FILE_INT, FILE_NONE, 1, A_STARP, &StarP[0].FeedbackFlag, 0,
             STARS_ONLY); /* TODO: actually short int */
#endif
  ...
  init_field(IO_DIVBALT, "DVBA", "MagneticFieldDivergenceAlternative", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP,
             &SphP[0].DivBalt, 0, GAS_ONLY); /* alternative version of divergence of magnetic field  */
  ...
  init_field(IO_AFLD, "AFLD", "MagneticVectorPotential", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, mhd_ct_read, 3, A_SPHP, &SphP[0].A[0], 0,
             GAS_ONLY); /* Magnetic Vector Potential */
#endif

#ifdef BECDM
  init_field(IO_PSIR, "PSIR", "PsiRe", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_P, &P[0].PsiRe, 0,
             2); /* output type 1 particles only */
  init_field(IO_PSII, "PSII", "PsiIm", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_P, &P[0].PsiIm, 0,
             2); /* output type 1 particles only */
#endif

#ifdef NON_IDEAL_MHD
#if defined(OHMIC_DIFFUSION) || defined(IMPLICIT_OHMIC_DIFFUSION)
  init_field(IO_CURLB, "CURB", "CurlB", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 3, A_SPHP, &SphP[0].CurlB[0], 0,
             GAS_ONLY); /* Magnetic Vector curl */
#endif
#endif
  ...
  init_field(IO_nHI, "nHI ", "NeutralFraction", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_SPHP, &SphP[0].nHI, 0,
             GAS_ONLY); /* neutral fraction */
#endif
  ...
  init_field(IO_nHeI, "nHe2", "HeNeutralFraction", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_SPHP, &SphP[0].nHeI, 0,
             GAS_ONLY); /* He neutral fraction */
  init_field(IO_nHeII, "nHe3", "HeIIFraction", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_NONE, 1, A_SPHP, &SphP[0].nHeII, 0,
             GAS_ONLY); /* HeII fraction */
#endif
  ...
  init_field(IO_ELECT, "elec", "elect", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].elec, 0,
             GAS_ONLY); /* electron abundance */
  init_field(IO_HI, "HI  ", "HI", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].HI, 0,
             GAS_ONLY); /* neutral hydrogen abundance */
  init_field(IO_HII, "HII ", "HII", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].HII, 0,
             GAS_ONLY); /* ionized hydrogen abundance */
  init_field(IO_HeI, "HeI ", "HeI", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].HeI, 0,
             GAS_ONLY); /* neutral Helium */
  init_field(IO_HeII, "HeII", "HeII", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].HeII, 0,
             GAS_ONLY); /* singly ionized Helium */
  init_field(IO_HeIII, "He3I", "HeIII", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].HeIII, 0,
             GAS_ONLY); /* doubly ionised Helium */
  init_field(IO_H2I, "H2I ", "H2I", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].H2I, 0,
             GAS_ONLY); /* H2 molecule */
  init_field(IO_H2II, "H2II", "H2II", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_SPHP, &SphP[0].H2II, 0,
             GAS_ONLY); /* ionised H2 molecule */
  ...
  init_field(IO_SECONDORDERMASS, "SOMA", "2LTP-Mass", MEM_NONE, FILE_NONE, FILE_MY_IO_FLOAT, 1, A_NONE, 0, io_func_secondordermass,
             GAS_ONLY);
#endif
  ...
  init_field(IO_LOCFBEVENT, "FBDO", "FeedbackDone", MEM_MY_FLOAT, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_P, &P[0].FeedbackDone, 0,
             STARS_ONLY); /* number of feedback events done */
#endif

#ifdef OUTPUT_STICKYFLAGS
  init_field(IO_STKY, "STKY", "StickyFlag", MEM_INT, FILE_INT, FILE_INT, 1, A_SPHP, &SphP[0].StickyFlag, 0,
             GAS_ONLY); /* Flag for sticky cells */
#endif
  ...
  init_field(IO_Z, "Z   ", "Metallicity", MEM_NONE, FILE_MY_IO_FLOAT, FILE_MY_IO_FLOAT, 1, A_NONE, 0, io_func_metallicity,
             GAS_AND_STARS); /* Gas and star metallicity */
#endif
