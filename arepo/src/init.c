...
static void process_initial_particle_positions();
...
int init(void)
{
  ...
  assert(RestartFlag != RESTART_RESTART);
...
#ifdef USE_SFR
      if(RestartFlag == RESTART_IC && P[i].Type == 0)
        SphP[i].Sfr = 0;
#endif
      ...
      for(i = 0; i < NumGas; i++)
        {
          ...
          SphP[i].ChimesGasVars.hydro_timestep = min(3.16e13 / SphP[i].ChimesGasVars.nH_tot, 3.16e16);
  ...
  if(RestartFlag == RESTART_SIMPLEX)
    {
      ...
      sx_evolve_pp();
#else
      mpi_printf("Compile with flag SIMPLEX!\n");
#endif
      return 0;
    }
  ...
  for(i = 0; i < NumPart; i++)
    {
      ...
      if(ids[i] == 0 && P[i].Type == 0 && P[i].Mass != 0)
        terminate("Particle %d has null ID, but is not tagged for removal since mass = %g in not null", i, P[i].Mass);
#endif
    }
...
/* set unused coordinate values in 1d and 2d simulations to zero; this is needed for correct interfaces */
  ...
  double box[3] = {All.BoxSize, All.BoxSize, All.BoxSize};
