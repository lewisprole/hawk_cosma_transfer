  ...
  for(iface = 0; iface < Mesh.Nvf; iface++)
    {
      ...
      double surfacearea = dmax(get_surface_area_of_point(p1), get_surface_area_of_point(p2));
...
static double streaming_advection_calc_flux(struct streaming_state *st_L, struct streaming_state *st_R, int iface)
{
  ...
  double S_L = dmin(-st_L->vel_alfven * fd->sign * fd->bfld[0], -st_R->vel_alfven * fd->sign * fd->bfld[0]);
  double S_R = dmax(-st_L->vel_alfven * fd->sign * fd->bfld[0], -st_R->vel_alfven * fd->sign * fd->bfld[0]);
...
void streaming_advection_explicit()
{
  TIMER_START(CPU_CR_STREAMING_SOLVE_EXPLICIT);
      ...
      if(ThisTask == task || !WroteSnapThisTimestep)
        {
          ...
          if(WroteSnapThisTimestep)
            {
              char buf[200];
              sprintf(buf, "%s/crstream%03d.txt", All.OutputDir, All.SnapshotFileCount - 1);
          ...
          for(iface = 0; iface < Mesh.Nvf; iface++)
            {
              ...
              double crEnergyDensityMax = dmax(pCREnergy[Mesh.VF[iface].p1], pCREnergy[Mesh.VF[iface].p2]);
              ...
              double area_fraction = fd->area * fabs(fd->bfld[0]) /
                                     dmax(get_active_area_of_point(Mesh.VF[iface].p1), get_active_area_of_point(Mesh.VF[iface].p2));
              double dt_max =
              ...
              if(dt_max < 0)
                ...
                advection_fraction = dmin(dt_max / fd->dt, 1.);
  ...
  for(iface = 0; iface < Mesh.Nvf; iface++)
    {
      ...
      ) ); rel = dmax( rel, 1.0 ); rel = dmin( rel, 1e5 );
      ...
      */
      ...
      gradient = dmax( gradient, 1e-3 * fd->crEnergyDensity / sqrt(dist2) );
      ...
      */
      ...
      gradient = dmax(gradient, 1e-5 * fd->crEnergyDensity / sqrt(dist2));
...
void streaming_diffusion_implicit()
{
