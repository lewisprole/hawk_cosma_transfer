      ...
      if(DumpFlag != 4)
        {
              ...
              if(ThisTask == 0)
                {
                  ...
                  if(subbox_flag > 0)
                    {
                      sprintf(buf, "%s", All.HCOutput_Directory);
                      mkdir(buf, 02755);
                      sprintf(buf, "%s/snipdir_%03d", All.HCOutput_Directory, num);
                      mkdir(buf, 02755);
                    }
#endif
                  ...
                  if(subbox_flag > 0)
                    {
                      ...
                      sprintf(alternative_fname, "%s/subbox%d/snapdir_subbox%d_%03d", AlternativeOutputDir, subbox_flag - 1,
                              subbox_flag - 1, num);
                    }
#endif
                  mkdir(alternative_fname, 02755);
          ...
          if(subbox_flag > 0)
            {
              if(All.NumFilesPerSnapshot > 1)
                sprintf(buf, "%s/subbox%d/snapdir_subbox%d_%03d/%s_subbox%d_%03d.%d", All.OutputDir, subbox_flag - 1, subbox_flag - 1,
                        num, All.SnapshotFileBase, subbox_flag - 1, num, filenr);
              else
          ...
          if(subbox_flag > 0)
            {
              if(All.NumFilesPerSnapshot > 1)
                sprintf(alternative_fname, "%s/subbox%d/snapdir_subbox%d_%03d/%s_subbox%d_%03d.%d", AlternativeOutputDir,
          ...
          if(subbox_flag > 0)
            {
              if(All.NumFilesPerSnapshot > 1)
                sprintf(buf, "%s/snipdir_%03d/snipshot_%03d.%d", All.HCOutput_Directory, num, num, filenr);
              else
                sprintf(buf, "%s/snipshot_%03d", All.HCOutput_Directory, num);
            }
#endif
  ...
  switch(blocknr)
    {
      ...
      case IO_AGE:
        for(i = 0; i < NTYPES; i++)
          if((All.ICFormat == SNAP_FORMAT_HDF5 && (i == 0 || i == 1 || i == 5)) || (All.ICFormat != SNAP_FORMAT_HDF5 && i != 4))
            typelist[i] = 0;
        return header.npart[4] + (All.ICFormat == SNAP_FORMAT_HDF5 ? (header.npart[2] + header.npart[3]) : 0);
        break;
      ...
      case IO_Z:
        for(i = 0; i < NTYPES; i++)
          {
            if((All.ICFormat == SNAP_FORMAT_HDF5 && (i == 1 || i == 5)) || (All.ICFormat != SNAP_FORMAT_HDF5 && (i != 0 && i != 4)))
              typelist[i] = 0;
            else
              typelist[i] = 1;
          }
        return header.npart[0] + header.npart[4] + (All.ICFormat == SNAP_FORMAT_HDF5 ? (header.npart[2] + header.npart[3]) : 0);
        break;
...
/* the modules have to be in the same order as the corresponding fields in the ICs */
      ...
      if(RestartFlag == RESTART_IC && blocknr > IO_U && blocknr != IO_BFLD)
#else
      if(RestartFlag == RESTART_IC)
#endif            /* READ_LEGACY_ICS */

        if(RestartFlag == RESTART_IC && blocknr > IO_HM)
#endif            /* CHEMISTRY */
#endif            /* CR_IC */
...

{
  ...
  if(rih)
    {
      sprintf(buf, "%s/rih_%s_%03d", All.OutputDir, id1, num);
      *rih = open_file(buf);
    }
  if(hrih)
    {
      sprintf(buf, "%s/hrih_%s_%03d", All.OutputDir, id1, num);
      *hrih = open_file(buf);
    }
  if(sxrates)
    {
      sprintf(buf, "%s/sxrates_%s_%03d", All.OutputDir, id1, num);
