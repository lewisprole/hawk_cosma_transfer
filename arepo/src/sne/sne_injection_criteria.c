...
int check_for_additional_sne(int sinkidx, int idx, int n_thistimeAlreadyDone)
{
  ...
  mpi_printf("Checking sink %d for additional SNe\n", SinkP[sinkidx].ID);
...
#else
  terminate("If you want SN feedback coupled with sink particles recompile the code with SINK_PARTICLES and SINK_PARTICLES_FEEDBACK, otherwise change injection criterion.\n");
#endif
  return NO_SNE;
}
  ...
  if(Wherewasi < 0)
    {
      ...
      SinkP[Sink_about_to_explode].explosion_time[Star_about_to_explode] = SinkP[Sink_about_to_explode].explosion_time[SinkP[Sink_about_to_explode].N_sne-1];
#ifdef POPIII_SNE
      SinkP[Sink_about_to_explode].stellar_mass[Star_about_to_explode] = SinkP[Sink_about_to_explode].stellar_mass[SinkP[Sink_about_to_explode].N_sne-1];
#endif
    ...
    mpi_printf("SINK_PARTICLES_FEEDBACK EXPLOSION: we just exploded an additional SN of sink %d\n", SinkP[Sink_about_to_explode].ID);
...
enum SNE_type sne_sinks_and_SNIa_TIMING()
{
  if(sne_random_TIMING())
...
enum SNE_type sne_poisson_sampling_SNII_AND_SNIa_TIMING()
{
#ifdef GALPOT
  if (NSNetot > 0)
    return SNE_POISSON;
...
enum SNE_type sne_sinks_and_SNIa_McMillan_TIMING()
{
  if(sne_random_TIMING())
    return SNE_GENERAL_DISTRIBUTION;
