 ...
 *  \param *total_n this will hold the total cells found on any processor, this should result equal to sn_size * All.SNEMinimalParticleNumber
 *  \param **indices here we will store the indices of the local particles found
 *  \param sn_size (>=1.0) increase the size of the injection region for very energetic SNe (i.e. E_51 > 1.0, e.g. PISNe). This is supposed to avoid heating the gas too much by thermal energy injection.
 *
 */
...
void sne_log(double sne_pos[3], int n, int injection_scheme, double radius,
             double mean_density, double mean_temperature, int n_tracer, double sn_mass, double sn_E51)
{
  if(ThisTask == 0)
    {
      fprintf(FdSNe, "%e, %e, %e, %e, %d, %d, %e, %e, %e, %d, %e, %e \n", All.Time, sne_pos[0], sne_pos[1],
          sne_pos[2], n, injection_scheme, radius, mean_density, mean_temperature, n_tracer, sn_mass, sn_E51);
      myflush(FdSNe);
    }
}
...
void inject_mass(enum SNE_type sne_type, int local_n, int indices[], double total_volume, double *total_mass, double *mean_density, double sn_mass)
{
  switch(sne_type)
    {
      case SNE_SINK:
...
void sne_sink_feedback_RETURN_MASS(int local_n, int indices[], double total_volume, double *total_mass, double *mean_density, double SN_mass)
{
  ...
  if(ThisTask == candidate_task)
    {
      ...
      if(P[particle_index].ID != SinkP[Sink_about_to_explode].ID)
        warn("Something is wrong, the sink is indexing the wrong particle, check maybe domain_rearrange.c where we eliminate sinks if we messed up the indexing there\n");
...
#ifdef GALPOT
void find_total_stellar_and_gas_mass()
{
