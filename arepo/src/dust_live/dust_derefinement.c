...
void do_derefinement_transfer(struct derefineimported_data *in)
{
  ...
  DTP(target).BinMassChgTau    = dmin(DTP(target).BinMassChgTau, in->BinMassChgTau);
  DTP(target).OldBinMassChgTau = dmin(DTP(target).OldBinMassChgTau, in->OldBinMassChgTau);
