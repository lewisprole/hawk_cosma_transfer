 ...
 *  \param RestartFlag[in] Mode in which Arepo is called
                     ...
                     double *xmin, double *xmax, double *ymin, double *ymax, double *zmin, double *zmax, int *weight_flag)
{
...
#else
  if(!((RestartFlag == RESTART_SLICE && argc == 14) || (RestartFlag == RESTART_PROJECTION && (argc == 15 || argc == 16))
#ifdef SUBBOX_SNAPSHOTS
       || (RestartFlag == RESTART_PROJECTION && (argc == 16 || argc == 17))
#endif
       || (RestartFlag == RESTART_PROJECTION_GRID_RAYTRACING && argc == 15)))
    {
      mpi_printf(
          "\nwrong parameters found: Expecting:\n Call with: <ParameterFile> %d <SnapNum> <pixelsX> <pixelsY> <axisX> <axisY> <axisZ> "
          " <xmin> <xmax> <ymin> <ymax> %s\n",
          RestartFlag, RestartFlag == RESTART_SLICE ? "<zval>" : "<zmin> <zmax>");
  ...
  *zmin = atof(argv[13]);
  ...
  if(RestartFlag == RESTART_PROJECTION || RestartFlag == RESTART_PROJECTION_GRID_RAYTRACING)
    {
...
#else
  if(gradients_flag == 1)
    sprintf(buf, "%s/proj_density_field_x%d_y%d_%03d", All.OutputDir, num, xaxis, yaxis);
  else if(gradients_flag == 0)
    sprintf(buf, "%s/proj_density_field_nograds_x%d_y%d_%03d", All.OutputDir, num, xaxis, yaxis);
#endif
  else
