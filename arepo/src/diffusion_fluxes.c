...
of the advectve fluxes*/
{
  ...
  flux->momentum[0] +=
      (geom->nx * (fac1 * st_face->vel_grad[0][0] - fac2 * (st_face->vel_grad[1][1] + st_face->vel_grad[2][2]) + fac3) +
       geom->ny * dyn_visc * (st_face->vel_grad[0][1] + st_face->vel_grad[1][0]) +
       geom->nz * dyn_visc * (st_face->vel_grad[0][2] + st_face->vel_grad[2][0]));

  flux->momentum[1] +=
      (geom->ny * (fac1 * st_face->vel_grad[1][1] - fac2 * (st_face->vel_grad[0][0] + st_face->vel_grad[2][2]) + fac3) +
       geom->nx * dyn_visc * (st_face->vel_grad[0][1] + st_face->vel_grad[1][0]) +
       geom->nz * dyn_visc * (st_face->vel_grad[1][2] + st_face->vel_grad[2][1]));

  flux->momentum[2] +=
      (geom->nz * (fac1 * st_face->vel_grad[2][2] - fac2 * (st_face->vel_grad[0][0] + st_face->vel_grad[1][1]) + fac3) +
       geom->nx * dyn_visc * (st_face->vel_grad[0][2] + st_face->vel_grad[2][0]) +
       geom->ny * dyn_visc * (st_face->vel_grad[1][2] + st_face->vel_grad[2][1]));
...
#ifdef THERMAL_CONDUCTION
  flux->energy +=
      -All.ThermalConductivity * (st_face->dTemp[0] * geom->nx + st_face->dTemp[1] * geom->ny + st_face->dTemp[2] * geom->nz);
#endif
}
...
void face_get_scalar_diffusion_fluxes(struct state_face *st_face, struct fluxes *flux, struct geometry *geom)
{
  ...
  flux->tracer = All.TracerDiffusivity * (st_face->dConservedTracer[0] * geom->nx + st_face->dConservedTracer[1] * geom->ny +
                                          st_face->dConservedTracer[2] * geom->nz);
