  ...
  for(i = 0, start = 0; i < nimport; i++)
    {
      ...
      if(FOF_GList[start].MinID != get_FOF_GList[i].MinID)
        terminate(
            "FOF_GList[start].MinID != get_FOF_GList[i].MinID start=%d i=%d FOF_GList[start].MinID=%" MYIDTYPE_PRI " get_FOF_GList[i].MinID=%" MYIDTYPE_PRI "\n",
  ...
  for(i = 0, start = 0, Nids = 0; i < NgroupsExt; i++)
    {
      ...
      if(FOF_PList[start].MinID != FOF_GList[i].MinID)
        terminate("FOF_PList[start=%d].MinID=%" MYIDTYPE_PRI " != FOF_GList[i=%d].MinID=%" MYIDTYPE_PRI, start, FOF_PList[start].MinID, i, FOF_GList[i].MinID);
