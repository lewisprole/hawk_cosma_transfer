...
void fof_subfind_exchange(MPI_Comm Communicator)
{
...
#endif
#ifdef DUST_LIVE
  int old_AllMaxPartDust = All.MaxPartDust;
#endif
      ...
      do
        {
          ...
          if(type == DUST_LIVE)
            {
              if(max_loaddust >= All.MaxPartDust - 0.5 * ALLOC_STARBH_ROOM * (All.TotNumGas / NTask))
                {
  ...
  if(max_loaddust < old_AllMaxPartDust)
    max_loaddust = old_AllMaxPartDust;
