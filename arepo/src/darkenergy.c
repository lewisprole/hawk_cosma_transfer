...
void fwa_init(void)
{
  ...
  if((fd = fopen(All.DarkEnergyFile, "r")))
    {
...
#endif
                {
                  mpi_printf("Wrong syntax in file `%s', line %d\n", All.DarkEnergyFile, count);
                  endrun();
                }
              a = 1. / atof(buf1);
              if(a == 0.0 && count == 1)
      ...
      if(count >= ANZ_W_A_IN - 1)
        {
          mpi_printf("File `%s' contains to many datapoints, increase ANZ_W_A_IN !\n", All.DarkEnergyFile);
          endrun();
        }
      if(count <= 2)
        {
          mpi_printf("File `%s' has to less Data Points (%d) !\n", All.DarkEnergyFile, count);
          endrun();
        }
  ...
  mpi_printf("Integrating w(a) finisched.\n");
}
