...
/*
 * ! \file implicit_ohmic_diffusion_cn_noperiodic.c
...
static inline double boundaryX(double dx)
{
  if(dx < -boxHalf_X)
    dx += boxSize_X;
  if(dx > boxHalf_X)
...
static inline double boundaryY(double dy)
{
  if(dy < -boxHalf_Y)
    dy += boxSize_Y;
  if(dy > boxHalf_Y)
...
static inline double boundaryZ(double dz)
{
  if(dz < -boxHalf_Z)
    dz += boxSize_Z;
  if(dz > boxHalf_Z)
...
void set_coeff(HYPRE_IJMatrix *A, HYPRE_IJVector *x, HYPRE_IJVector *b, int *fld_offsets, double dt, int direction)
{
  ...
  double boxsize[3] = {boxSize_X, boxSize_Y, boxSize_Z};
          ...
          for(int k = 0; k < 3; k++)
            {
              ...
              if((p1[k] - p0[k]) > 0.5 * boxsize[k] || (p1[k] - p0[k]) < -0.5 * boxsize[k])
                {
                  is_boundary = 1;
                }
            }

          dx = boundaryX(p1[0] - p0[0]);
          dy = boundaryY(p1[1] - p0[1]);
          dz = boundaryZ(p1[2] - p0[2]);
