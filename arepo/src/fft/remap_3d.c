...
void remap_3d(double *in, double *out, double *buf, struct remap_plan_3d *plan)
{
  ...
  for(irecv = 0; irecv < plan->nrecv; irecv++)
    MPI_Irecv(&scratch[plan->recv_bufloc[irecv]], plan->recv_size[irecv], MPI_DOUBLE, plan->recv_proc[irecv], 0, plan->comm,
              &plan->request[irecv]);
  ...
  for(isend = 0; isend < plan->nsend; isend++)
    {
      ...
      MPI_Send(plan->sendbuf, plan->send_size[isend], MPI_DOUBLE, plan->send_proc[isend], 0, plan->comm);
    }
...
struct remap_plan_3d *remap_3d_create_plan(MPI_Comm comm, int in_ilo, int in_ihi, int in_jlo, int in_jhi, int in_klo, int in_khi,
                                           int out_ilo, int out_ihi, int out_jlo, int out_jhi, int out_klo, int out_khi, int nqty,
                                           int permute, int memory, int precision)
{
  ...
  if(nsend)
    {
      ...
      if(plan->send_offset == NULL || plan->send_size == NULL || plan->send_proc == NULL || plan->packplan == NULL)
        return NULL;
    }
      ...
      if(remap_3d_collide(&in, &array[iproc], &overlap))
        {
          ...
          plan->send_offset[nsend] =
              nqty * ((overlap.klo - in.klo) * in.jsize * in.isize + ((overlap.jlo - in.jlo) * in.isize + overlap.ilo - in.ilo));
  ...
  if(nrecv)
    {
      ...
      if(plan->recv_offset == NULL || plan->recv_size == NULL || plan->recv_proc == NULL || plan->recv_bufloc == NULL ||
      ...
      if(nrecv > 0)
        {
          if(precision == 1)
            ...
            plan->scratch = (double *)malloc(nqty * out.isize * out.jsize * out.ksize * sizeof(double));
...
int remap_3d_collide(struct extent_3d *block1, struct extent_3d *block2, struct extent_3d *overlap)
{
  ...
  if(overlap->ilo > overlap->ihi || overlap->jlo > overlap->jhi || overlap->klo > overlap->khi)
    return 0;
