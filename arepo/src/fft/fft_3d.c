...
void fft_3d(FFT_DATA *in, FFT_DATA *out, int flag, struct fft_plan_3d *plan)
{
  ...
  if(plan->pre_plan)
    {
      if(plan->pre_target == 0)
        ...
        copy = plan->copy;
      remap_3d((double *)in, (double *)copy, (double *)plan->scratch, plan->pre_plan);
      data = copy;
    }
  else
    data = in;
...
#ifdef FFT_T3E
  for(offset = 0; offset < total; offset += length)
    FFT_1D(&flag, &length, &scalef, &data[offset], &data[offset], plan->coeff1, plan->work1, &isys);
#endif
#ifdef FFT_FFTW
  if(flag == -1)
  ...
  if(plan->mid1_target == 0)
    ...
    copy = plan->copy;
  remap_3d((double *)data, (double *)copy, (double *)plan->scratch, plan->mid1_plan);
  data = copy;
...
#ifdef FFT_T3E
  for(offset = 0; offset < total; offset += length)
    FFT_1D(&flag, &length, &scalef, &data[offset], &data[offset], plan->coeff2, plan->work2, &isys);
#endif
#ifdef FFT_FFTW
  if(flag == -1)
  ...
  if(plan->mid2_target == 0)
    ...
    copy = plan->copy;
  remap_3d((double *)data, (double *)copy, (double *)plan->scratch, plan->mid2_plan);
  data = copy;
...
#ifdef FFT_T3E
  for(offset = 0; offset < total; offset += length)
    FFT_1D(&flag, &length, &scalef, &data[offset], &data[offset], plan->coeff3, plan->work3, &isys);
#endif
#ifdef FFT_FFTW
  if(flag == -1)
  ...
  if(plan->post_plan)
    remap_3d((double *)data, (double *)out, (double *)plan->scratch, plan->post_plan);
...
struct fft_plan_3d *fft_3d_create_plan(MPI_Comm comm, int nfast, int nmid, int nslow, int in_ilo, int in_ihi, int in_jlo, int in_jhi,
                                       int in_klo, int in_khi, int out_ilo, int out_ihi, int out_jlo, int out_jhi, int out_klo,
                                       int out_khi, int scaled, int permute, int *nbuf)
{
  ...
  if(remapflag == 0)
  ...
  else
    {
      ...
      plan->pre_plan = remap_3d_create_plan(comm, in_ilo, in_ihi, in_jlo, in_jhi, in_klo, in_khi, first_ilo, first_ihi, first_jlo,
                                            first_jhi, first_klo, first_khi, FFT_PRECISION, 0, 0, 2);
  ...
  plan->mid1_plan = remap_3d_create_plan(comm, first_ilo, first_ihi, first_jlo, first_jhi, first_klo, first_khi, second_ilo,
                                         second_ihi, second_jlo, second_jhi, second_klo, second_khi, FFT_PRECISION, 1, 0, 2);
  ...
  plan->mid2_plan = remap_3d_create_plan(comm, second_jlo, second_jhi, second_klo, second_khi, second_ilo, second_ihi, third_jlo,
                                         third_jhi, third_klo, third_khi, third_ilo, third_ihi, FFT_PRECISION, 1, 0, 2);
  ...
  if(permute == 2 && out_ilo == third_ilo && out_ihi == third_ihi && out_jlo == third_jlo && out_jhi == third_jhi &&
  ...
  if(remapflag == 0)
    ...
    {
      plan->post_plan = remap_3d_create_plan(comm, third_klo, third_khi, third_ilo, third_ihi, third_jlo, third_jhi, out_klo, out_khi,
                                             out_ilo, out_ihi, out_jlo, out_jhi, FFT_PRECISION, (permute + 1) % 3, 0, 2);
  ...
  *nbuf = copy_size + scratch_size;
  ...
  if(plan->coeff1 == NULL || plan->coeff2 == NULL || plan->coeff3 == NULL)
    return NULL;
  ...
  if(scaled == 0)
  ...
  else
    {
      ...
      plan->normnum = (out_ihi - out_ilo + 1) * (out_jhi - out_jlo + 1) * (out_khi - out_klo + 1);
    }
  ...
  if(fftflag)
    {
      if(me == 0)
        printf("ERROR: FFTs are not power of 2,3,5\n");
      return NULL;
    }
  ...
  if(plan->work1 == NULL || plan->work2 == NULL || plan->work3 == NULL)
    return NULL;
  ...
  plan->plan_fast_forward  = fftw_create_plan(nfast, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  plan->plan_fast_backward = fftw_create_plan(nfast, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
  ...
  if(nmid == nfast)
    ...
    {
      plan->plan_mid_forward  = fftw_create_plan(nmid, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
      plan->plan_mid_backward = fftw_create_plan(nmid, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
    }
  ...
  if(nslow == nfast)
    ...
    {
      plan->plan_slow_forward  = fftw_create_plan(nslow, FFTW_FORWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
      plan->plan_slow_backward = fftw_create_plan(nslow, FFTW_BACKWARD, FFTW_ESTIMATE | FFTW_IN_PLACE);
    }
...
#endif
#ifdef FFT_FFTW
  if(plan->plan_slow_forward != plan->plan_mid_forward && plan->plan_slow_forward != plan->plan_fast_forward)
    {
