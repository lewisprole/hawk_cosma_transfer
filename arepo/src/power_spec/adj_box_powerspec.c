...
double adj_box_powerspec_obtain_fields(void)
{
  ...
  large_array_offset i, n,
      Ncount = ((large_array_offset)nslab_x) * (FOURIER_GRID * FOURIER_GRID); /* number of grid points on the local slab */
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) / (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(data_in) +
                                                          sizeof(data_out) + sizemax(sizeof(data_in), sizeof(data_out))));
