...
#pragma omp parallel for private(i, j) schedule(dynamic)
  ...
  */
      ...
      press = godunov_flux_3d_hlle_general_relativity(&state_L, &state_R, vel_face_turned, &state_face, &fluxes, &geom, VF[i].cx,
                                                      VF[i].cy, VF[i].cz);
#else
#ifdef RIEMANN_GAMMA
...
#if defined(RIEMANN_HLLC) || defined(RIEMANN_ROSUNOV) || defined(RIEMANN_HLLD) || defined(RIEMANN_HLL) ||                          \
    defined(SPECIAL_RELATIVITY) ||                                                                                                 \
    defined(GENERAL_RELATIVITY) /* for non-exact Riemann solver, fluxes are already computed in the local frame, so convert to lab \
      ...
      flux_convert_to_lab_frame(&state_L, &state_R, vel_face_turned, &fluxes, &state_face);
#endif
      ...
      face_get_eddy_viscosity(&state_L, &state_R, &state_face, &fluxes);
      face_get_reconstructed_gradients(&state_L, &state_R, &state_face, &fluxes);
      face_add_sgs_stress_tensor_fluxes(&state_face, &geom, &fluxes);
#endif
#endif
...
#endif
#ifdef MHD_DEDNER
          fluxes.Psi *= sqrtatime / (atime * atime);
          state_face.Psi *= sqrtatime / (atime * atime);
#endif
#endif
              ...
              if(DP[q].task == ThisTask)
                {
...
#ifdef MHD_DEDNER
                  SphP[p].PsiConserved += dir * fluxes.Psi;
#endif
#ifdef MHD_DEDNER_VARIABLE_SPEED
                  SphP[p].Energy += dir * state_face.Psi *
                                    (geom.nx * (state_center->Bx + delta_time->Bx) + geom.ny * (state_center->By + delta_time->By) +
                                     geom.nz * (state_center->Bz + delta_time->Bz)) *
                                    All.cf_atime * All.cf_atime * All.cf_atime * All.cf_atime * All.cf_atime;
                  SphP[p].BConserved[0] += dir * state_face.Psi * geom.nx * All.cf_atime * All.cf_atime;
                  SphP[p].BConserved[1] += dir * state_face.Psi * geom.ny * All.cf_atime * All.cf_atime;
                  SphP[p].BConserved[2] += dir * state_face.Psi * geom.nz * All.cf_atime * All.cf_atime;
                  SphP[p].PsiConserved += dir * (state_center->rho + delta_time->rho) * state_face.Bx * state_center->DednerSpeed *
                  ...
                  double dEnergy = dir * (Bx * Velx + By * Vely + Bz * Velz) * state_face.Bx * atime;
#ifdef MHD_POWELL_ENERGYLIMITER
                  ...
                  for(m = 0; m < N_Scalar; m++)
                    {
                      //                  assert(*(MyFloat *) (((char *) (&SphP[p])) + scalar_elements[m].offset_mass) + dir *
                      //                  fluxes.scalars[m]>=0);
...
#ifdef CHIMES_ADVECT_ABUNDANCES
                  for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                    SphP[p].ChimesGasVars.IonAdvect[abunIndex] += dir * fluxes.IonFluxes[abunIndex];
                  SphP[p].ChimesGasVars.HtotAdvect += dir * fluxes.HtotFlux;
#endif
                }
              else
                {
...
#ifdef MHD_DEDNER
                  FluxList[Nflux].dPsi    = dir * fluxes.Psi;
#endif
#ifdef MHD_DEDNER_VARIABLE_SPEED
                  FluxList[Nflux].dEnergy +=
                      ...
                      (geom.nx * (state_center->Bx + delta_time->Bx) + geom.ny * (state_center->By + delta_time->By) +
                       geom.nz * (state_center->Bz + delta_time->Bz)) *
                      All.cf_atime * All.cf_atime * All.cf_atime * All.cf_atime * All.cf_atime;
                  FluxList[Nflux].dB[0] += dir * state_face.Psi * geom.nx * All.cf_atime * All.cf_atime;
                  FluxList[Nflux].dB[1] += dir * state_face.Psi * geom.ny * All.cf_atime * All.cf_atime;
                  FluxList[Nflux].dB[2] += dir * state_face.Psi * geom.nz * All.cf_atime * All.cf_atime;
                  FluxList[Nflux].dPsi += dir * (state_center->rho + delta_time->rho) * state_face.Bx * state_center->DednerSpeed *
                                          state_center->DednerSpeed / (All.cf_atime * All.cf_atime * All.cf_atime);
#endif
#ifdef MHD_CT
...
#ifdef CHIMES_ADVECT_ABUNDANCES
                  for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                    FluxList[Nflux].IonAdvect[abunIndex] = dir * fluxes.IonFluxes[abunIndex];

                  FluxList[Nflux].HtotAdvect = dir * fluxes.HtotFlux;
#endif
                  Nflux++;
                }
            }
...
void face_do_time_extrapolation(struct state *delta, struct state *st, double atime)
{
  ...
  delta->Psi=-dt_half*((st->DednerSpeed*st->DednerSpeed*(grad->dB[0][0]+grad->dB[1][1]+grad->dB[2][2])/(atime*atime))+(st->Psi*0.8*st->DednerSpeed/pow(st->volume,1./3.)) +0.5*st->Psi*(grad->dvel[0][0]+grad->dvel[1][1]+grad->dvel[2][2])/(atime*atime));
#endif
#ifdef MHD_CT
  delta->Ax =
