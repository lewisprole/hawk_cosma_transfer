...
static double shift(double pos, int dim)
{
  pos -= All.GlobalDisplacementVector[dim];
...
void print_particle_info(int i)
{
  printf("Task=%d, ID=%llu, Type=%d, TimeBinGrav=%d, TimeBinHydro=%d, Mass=%g, pos=%g|%g|%g, vel=%g|%g|%g\n", ThisTask,
         (unsigned long long)P[i].ID, P[i].Type, P[i].TimeBinGrav, P[i].TimeBinHydro, P[i].Mass, shift(P[i].Pos[0], 0),
         shift(P[i].Pos[1], 1), shift(P[i].Pos[2], 2), P[i].Vel[0], P[i].Vel[1], P[i].Vel[2]);
#ifdef PMGRID
