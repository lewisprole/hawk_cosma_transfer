...
void apply_barotropic_eos(void) /* normal cooling routine when star formation is disabled */
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
	...
	*/
          ...
          /*Masahiro*/
	double mp_lewis=1.672621777e-24;
        double mu_lewis=2.4;
        double kb_lewis=1.3806488e-16;


        double k0=kb_lewis*10/(mu_lewis*mp_lewis);
        double T1=k0*mu_lewis*mp_lewis/kb_lewis;

        double k1=kb_lewis*T1/(mu_lewis*mp_lewis*pow(2e10*mu_lewis*mp_lewis, 1.6666666666666667-1));
        double T2=k1*mu_lewis*mp_lewis*pow(2e10*mu_lewis*mp_lewis, 1.6666666666666667-1)/kb_lewis;

        double k2=kb_lewis*T2/(mu_lewis*mp_lewis*pow(3e13*mu_lewis*mp_lewis, 1.4-1));
        double T3=k2*mu_lewis*mp_lewis*pow(3e13*mu_lewis*mp_lewis, 1.4-1)/kb_lewis;

        double k3=kb_lewis*T3/(mu_lewis*mp_lewis*pow(1e15*mu_lewis*mp_lewis, 1.1-1));
	double T4=k3*mu_lewis*mp_lewis*pow(1e15*mu_lewis*mp_lewis, 1.1-1)/kb_lewis;

	double k4=kb_lewis*T4/(mu_lewis*mp_lewis*pow(3e18*mu_lewis*mp_lewis, 2-1));
	
	

 
	if (SphP[i].Density<2e10*mu_lewis*mp_lewis)
	  SphP[i].Utherm= k0  / (GAMMA -1);
	
	if (SphP[i].Density<3e13*mu_lewis*mp_lewis && SphP[i].Density>=2e10*mu_lewis*mp_lewis)
	  SphP[i].Utherm= k1*   pow(SphP[i].Density, 1.6666666666666667 - 1) / (GAMMA -1);

	if (SphP[i].Density<1e15*mu_lewis*mp_lewis && SphP[i].Density>=3e13*mu_lewis*mp_lewis)
	  SphP[i].Utherm= k2*   pow(SphP[i].Density, 1.4 - 1) / (GAMMA -1);

	if (SphP[i].Density<3e18*mu_lewis*mp_lewis && SphP[i].Density>=1e15*mu_lewis*mp_lewis)
	  SphP[i].Utherm= k3*   pow(SphP[i].Density, 1.1 - 1) / (GAMMA -1);

	if (SphP[i].Density>3e18*mu_lewis*mp_lewis)
	  SphP[i].Utherm= k4*   pow(SphP[i].Density, 2 - 1) / (GAMMA -1);
