...
void apply_barotropic_eos(void) /* normal cooling routine when star formation is disabled */
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
	...
	*/
	...
	double c_lewis=190e2;
	double rho_cu=1.991e-18;
	double v_cu=36447.2682;
	double rho_c=3.84e-13/rho_cu;
	double rho_d=3.84e-8/rho_cu;
	double rho_e=3.84e-3/rho_cu;



	if (SphP[i].Density<rho_c)
	  SphP[i].Utherm=  pow(c_lewis/v_cu,2) / (GAMMA -1);
	
	if (SphP[i].Density<rho_d && SphP[i].Density>rho_c)
	  SphP[i].Utherm=  pow(c_lewis/v_cu,2) * (rho_c/SphP[i].Density) * pow(SphP[i].Density/rho_c,1.4)  / (GAMMA -1);

	if (SphP[i].Density<rho_e && SphP[i].Density>rho_d)
	  SphP[i].Utherm=  pow(c_lewis/v_cu,2) * (rho_c/SphP[i].Density) * pow(rho_d/rho_c,1.4) * pow(SphP[i].Density/rho_d,1.1) / (GAMMA -1);

	if (SphP[i].Density>rho_e)
	  SphP[i].Utherm=  pow(c_lewis/v_cu,2) * (rho_c/SphP[i].Density) * pow(rho_d/rho_c,1.4) * pow(rho_e/rho_d,1.1) *pow(SphP[i].Density/rho_e,1.6666666666666667) / (GAMMA -1);
