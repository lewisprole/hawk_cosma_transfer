...
int create_sink_particles(void)
{
  ...
  */
  ...
  if (All.ComovingIntegrationOn)
    {
      esupport = environment_out.ekin / (All.Time * All.Time) + environment_out.etherm; // conversion of kinetc energy to proper units
    }
  else
    {
  ...
  else
    {
      if((egrav <= 2.*esupport)  ||  (environment_out.divv >= 0) || (environment_out.diva >= 0))
        {
          ...
          mpi_printf("SINK_PARTICLES: Number density cgs %g Temperature %g average Temperature %g \n", 
              ...
              environment_out.etherm / environment_out.mass * All.UnitEnergy_in_cgs / All.UnitMass_in_g * 2.0/3.0 * 1.22 * PROTONMASS / BOLTZMANN);
	  ...
	  return sink_creation_success;
...
#endif
	    {
	      ...
	      warn("SINK_PARTICLES: Candidate density of %g is now super dense but not converging....... Forcing sink formation regardless of all checks! \n", candidate.Density);
	    }
	  else
	    {
	      sink_creation_success = 0;
