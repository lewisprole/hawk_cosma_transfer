 ...
 MPI_Bcast(&sne, sizeof(struct sne_buf), MPI_BYTE, candidate_task, MPI_COMM_WORLD);
  ...
  if((SinkP[sink_index].N_sne + sne.n_sne) > MAXSNE)
          ...
          if(SinkP[sink_index].MassStillToConvert[i] == 0.)
            {
              ...
              mpi_printf("SINK_PARTICLES_FEEDBACK: sink %d has N_sne = %d and just created another %d sne\n", SinkP[sink_index].ID,
  ...
  else
    {
      ...
      mpi_printf("SINK_PARTICLE_FEEDBACK: given an accreted mass of m = %g onto the sink ID = %d, I just created %d massive stars\n",
  ...
  for(int i=0; i<N_IMF_BINS; i++)
    {
      ...
      *n_sne += ni;
      mpi_printf("SINK_PARTICLE_FEEDBACK: drawing from IMF in bin %d mass in bin %g, no. of supernova %d, last entry mass array %g, mass to convert %g %g \n", i, mu, ni,massarr[*n_sne - 1],mass_to_convert,f[i]);
    }
#else
