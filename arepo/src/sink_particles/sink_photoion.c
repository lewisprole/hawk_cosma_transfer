...
double find_stromgren_radius_around_sink(int isink, int *local_n, int *total_n, int **indices)
{
  ...
  for(int i = 0; i<*local_n; i++)
    {
      ...
      storeLocalData[i].r2_ParticleSink = pow((P[idx].Pos[0]-sinkx), 2) + 
                                          pow((P[idx].Pos[1]-sinky), 2) + 
                                          pow((P[idx].Pos[2]-sinkz), 2);
  ...
  MPI_Allgatherv(&storeLocalData[0], *local_n * sizeof(struct collect_data), MPI_BYTE,
                 &storeGlobalData[0], bytecounts, byteoffset, MPI_BYTE, MPI_COMM_WORLD);
...
#if CHEMISTRYNETWORK == 1
          double yntot = (1.0 + ABHE - SphP[idx].TracAbund[IH2] + SphP[idx].TracAbund[IHP] + SphP[idx].TracAbund[IHEP] + 2.*SphP[idx].TracAbund[IHEPP]) * yn;
...
#endif
          double mom2 = SphP[idx].Momentum[0]*SphP[idx].Momentum[0] + SphP[idx].Momentum[1]*SphP[idx].Momentum[1] + SphP[idx].Momentum[2]*SphP[idx].Momentum[2];
