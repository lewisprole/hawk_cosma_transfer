...
double MolecularHFrac(int i)
{
  ...
  tau = 0.067 * ((All.UnitMass_in_g / SOLAR_MASS) * (pow(PARSEC / All.UnitLength_in_cm, 2.0))) * metallicity / SOLAR_METALLICITY *
  sigma;
  */
  ...
  gradD = sqrt(SphP[i].Grad.drho[0] * SphP[i].Grad.drho[0] + SphP[i].Grad.drho[1] * SphP[i].Grad.drho[1] +
          ...
          pow(a3inv, 0.66);  // LVS this shouldn't be ( )*a3inv * a3inv, but ainv^2;
  if(gradD > 0)
    {
