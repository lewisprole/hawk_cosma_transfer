...
void do_early_stellar_feedback(void)
{
          ...
          if(StarParticle[i].EarlyTotalEnergyReleased > 0)
            {
              ...
              momentum_diff   = sqrt(StarParticle[i].deltaEarlyMomentum[0] * StarParticle[i].deltaEarlyMomentum[0] +
                                   StarParticle[i].deltaEarlyMomentum[1] * StarParticle[i].deltaEarlyMomentum[1] +
                                   StarParticle[i].deltaEarlyMomentum[2] * StarParticle[i].deltaEarlyMomentum[2]);
              active_stars++;
            }
          else
            {
  ...
  mpi_printf(
      "FM_EARLY_FEEDBACK: kinetic feedback (this step)  ---> number of kicked cells = %d, active stars (with SN explosions) = %d, "
      "kicked cells / star = %g\n",
  ...
  mpi_printf(
      "FM_EARLY_FEEDBACK: kinetic feedback (this step)  ---> total mass to kick = %g, total kicked mass = %g, avg mass kicked = %g\n",
...
int early_stellar_feedback_evaluate(int target, int mode, int thread_id)
{
  ...
  if(tot_mass <= 0)
    ...
    {
      prob = 2.0 * EarlyTotalKinEnergyReleased /
             (velocity * velocity * tot_mass);    /*LVS: here velocity CANNOT be larger than set in parameterfile */
      velocity *= dmax(1.0, sqrt(prob) + 1.0e-6); /*LVS: here velocity CAN be larger than set in parameterfile $$ */
      prob = 2.0 * EarlyTotalKinEnergyReleased /
             (velocity * velocity * tot_mass); /*LVS: here velocity CAN be larger than set in parameter file $$ */
    }
