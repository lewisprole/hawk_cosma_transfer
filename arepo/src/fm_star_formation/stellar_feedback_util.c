...
int check_if_turbulent_energy_above_threshold(int i) { return (SphP[i].Uturb > All.SigmaThreshold ? 1 : 0); }

MyFloat dissipate_turbulent_energy(MyFloat Uturb_old, MyFloat dt) { return (Uturb_old * exp(-dt / All.DissipationTime)); }
#endif
...
MyDouble compute_total_SN_energy(int index, MyDouble Age, MyFloat* SN_Number)
{
  ...
  *SN_Number = 0.0;
  ...
  if(Age >= All.MinFeedAge && STP(index).FeedbackFlag > 0)
    {
      *SN_Number = STP(index).InitialMass * All.UnitMass_in_g * All.SNaePerUnitMass / SOLAR_MASS / All.HubbleParam /
                   All.FeedbackInjectionEvents;
