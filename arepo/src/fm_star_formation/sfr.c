...
double get_starformation_rate(int i)
{
  ...
  MyFloat temp = convert_u_to_temp(SphP[i].Utherm * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs,
                                   SphP[i].Density * All.cf_a3inv * All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam, &ne);
  if(temp > 1e3)
    rateOfSF *= 0.0;
#endif
  ...
  if(P[index].Type == 0)
    {
      ...
      if(prob > 1)
        {
          printf(
              "SFR: Warning, need to make a heavier star than desired. Task=%d prob=%g P[index].Mass=%g mass_of_star=%g "
              "mass_of_star_new=%g p=%g\n",
