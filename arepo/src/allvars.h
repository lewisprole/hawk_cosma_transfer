...
#if defined(NUCLEAR_NETWORK) && defined(NUCLEAR_NETWORK_DISABLE_BURNING_IN_SHOCK)
...
#if defined(MHD_DEDNER) && !defined(MHD_POWELL)
#ifndef MHD_DEDNER_WITHOUT_POWELL_OVERRIDE
#error \
...
#if defined(COSMIC_RAYS_STREAMING) || defined(COSMIC_RAYS_DIFFUSION) || defined(MONOTONE_CONDUCTION) || defined(VORONOI_MESHOUTPUT)
...
#define warn(...)                                                                                                                   \
  {                                                                                                                                 \
    char termbuf1[1000], termbuf2[1000];                                                                                            \
    sprintf(termbuf1, "WARNING: Code warning on task=%d, function %s(), file %s, line %d", ThisTask, __func__, __FILE__, __LINE__); \
    sprintf(termbuf2, __VA_ARGS__);                                                                                                 \
    printf("%s: %s\n", termbuf1, termbuf2);                                                                                         \
    myflush(stdout);                                                                                                                \
...
#define mymalloc(x, y) mymalloc_fullinfo(x, y, __func__, __FILE__, __LINE__, 0, NULL)
#define mymalloc_g(x, y) mymalloc_fullinfo(x, y, __func__, __FILE__, __LINE__, 0, callorigin)
#define mymalloc_clear(x, y) mymalloc_fullinfo(x, y, __func__, __FILE__, __LINE__, 1, NULL)
#define mymalloc_movable(x, y, z) mymalloc_movable_fullinfo(x, y, z, __func__, __FILE__, __LINE__, NULL)
#define mymalloc_movable_g(x, y, z) mymalloc_movable_fullinfo(x, y, z, __func__, __FILE__, __LINE__, callorigin)

#define myrealloc(x, y) myrealloc_fullinfo(x, y, __func__, __FILE__, __LINE__)
#define myrealloc_movable(x, y) myrealloc_movable_fullinfo(x, y, __func__, __FILE__, __LINE__)

#define myfree(x) myfree_fullinfo(x, __func__, __FILE__, __LINE__)
#define myfree_movable(x) myfree_movable_fullinfo(x, __func__, __FILE__, __LINE__)
...
#define mynew_movable(addr, name, T, n) \
  (T *)(mymalloc_movable_fullinfo((addr), name, (n) * sizeof(T), __func__, __FILE__, __LINE__, NULL))
...
/* calculate appropriate value of MAXSCALARS */
    ...
    defined(PASSIVE_SCALARS) || defined(DELAYED_COOLING_TURB) || defined(REFINEMENT_RPS) || defined(SGCHEM) || defined(EOS_OPAL) || \
    defined(COSMIC_RAYS) || defined(SGS_TURBULENCE) || defined(GFM_RPROCESS_CHANNELS) || defined(MRT) || defined(REFINEMENT_CGM)
...
#define MAXSCALARS                                                                                                     \
  (COUNT_EOS + COUNT_MHD_TES + COUNT_REFINE + COUNT_METALS + COUNT_STELLAR_EVOLUTION + COUNT_PASSIVE_SCALARS +         \
   COUNT_DELAYED_COOLING + COUNT_RPS + COUNT_SGCHEM + COUNT_CR + COUNT_SGS_T + COUNT_GFM_RPROCESS + COUNT_REFINE_CGM + \
   COUNT_MRT_IONS)
#endif
...
#define MAXGRADIENTS                                                                                                                \
  (COUNT_GRAD_DEFAULT + COUNT_GRAD_ENTR + COUNT_GRAD_UTHERM + COUNT_GRAD_TGCHEM + COUNT_GRAD_GAMMA + COUNT_GRAD_MHD +               \
   ...
   COUNT_CR + COUNT_FLD + COUNT_GRAD_SGS_T)
...
#define NGRID_BFIELD                                                                                                                \
  128 /* NGRID per dimension of the file specifying the B-field is hardcoded, so we can allocate stack memory in the All structure. \
       */
...
extern struct global_data_all_processes
{
...
#if !defined(FM_SFR) && !defined(LOCAL_FEEDBACK) && !defined(SFR_MCS) /* enable Springel & Hernquist model */
  ...
  /*  double EarlyFeedbackVelocity;  */ /**< the (constant) kick velocity indiced by early stellar feedback,deactivated now, using
                                           local escape speed */
#endif
...
#ifdef FM_EARLY_STAR_FEEDBACK
  MyDouble
      EarlyCumulativeFeedback; /* stores the total(cumulative) amount of energy input as early feedback throughout lifetime of star */
#endif
