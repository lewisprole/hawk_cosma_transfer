...
{
  // allocate memory for struct holding GroupCat properties for groups on this task
  // GroupCatLocal = NULL;
  if(Ngroups)
    GroupCatLocal =
        (struct group_catalogue *)mymalloc_movable(&GroupCatLocal, "GroupCatLocal", Ngroups * sizeof(struct group_catalogue));

  // import GroupCatLocal from GroupCat (task 0 only)
  int i, j;
  int *count = NULL;
  ...
  if(ThisTask == 0)
    {
      for(j = 0; j < Ngroups; j++)
        GroupCatLocal[j] = GroupCat[Group[j].GrNr];
        ...
        if(count[i])
          {
            ...
            MPI_Status status;
            MPI_Recv(GrNr, count[i], MPI_INT, i, TAG_N + i, MPI_COMM_WORLD, &status);

            GroupCatSend = (struct group_catalogue *)mymalloc("GroupCatSend", count[i] * sizeof(struct group_catalogue));

            for(j = 0; j < count[i]; j++)
              GroupCatSend[j] = GroupCat[GrNr[j]];

            MPI_Send(GroupCatSend, count[i] * sizeof(struct group_catalogue), MPI_BYTE, i, TAG_N + i, MPI_COMM_WORLD);
  ...
  if(Ngroups)  // this fct is redundant
    myfree_movable(GroupCatLocal);
}
                  ...
                  if(ngroups > 0)
                    {
                      ...
                      CommBuffer = (void *)mymalloc("CommBuffer", bytes_per_blockelement * ngroups);  // exactly adjusted
                  ...
                  if(nsubgroups > 0)
                    {
                      ...
                      CommBuffer = (void *)mymalloc("CommBuffer", bytes_per_blockelement * nsubgroups);  // exactly adjusted
