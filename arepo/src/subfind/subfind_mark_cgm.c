  ...
  for(int ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Force_Send_count[recvTask] > 0 || Force_Recv_count[recvTask] > 0)
          MPI_Sendrecv(&Tree_Points[Force_Recv_offset[recvTask]], Force_Recv_count[recvTask] * sizeof(struct treepoint_data),
                        MPI_BYTE, recvTask, TAG_DENS_A,
                        &export_Tree_Points[Force_Send_offset[recvTask]], Force_Send_count[recvTask] * sizeof(struct treepoint_data), MPI_BYTE,
                        recvTask, TAG_DENS_A,
