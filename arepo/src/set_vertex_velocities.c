...
#if defined(MHD) && defined(MHD_DEDNER_REDUCE_MESH_MOTIONS_IN_HIGH_DIVB)
      /* Reduce cell velocity to zero in case of high fractional divb */
      double bmag = sqrt(SphP[i].B[0]*SphP[i].B[0] + SphP[i].B[1]*SphP[i].B[1] + SphP[i].B[2]*SphP[i].B[2]);
      double brake_cell = dmax(0, 1 - 10 * SphP[i].DivB * get_cell_radius(i) / bmag);
      SphP[i].VelVertex[0] *= brake_cell;
      SphP[i].VelVertex[1] *= brake_cell;
      SphP[i].VelVertex[2] *= brake_cell;
#endif
    }
