...
#if defined(COOLING)
#if !defined(GRACKLE) && !defined(CHIMES)
...
update_radiation_state(dens * All.cf_a3inv, HYDROGEN_MASSFRAC, 0);
  ...
  // convert_u_to_temp(dmax(All.MinEgySpec, SphP[i].Utherm) * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs, dens * All.cf_a3inv *
  // All.UnitDensity_in_cgs * All.HubbleParam * All.HubbleParam, &ne); unew = dmax(All.MinEgySpec, SphP[i].Utherm);
...
#if defined(FM_RADIATION_FEEDBACK)  // LVS: keep T = 10^4
  if(dtcool1 == 0)
    {
