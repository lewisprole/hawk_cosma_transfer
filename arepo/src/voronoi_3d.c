...
#if defined(VORONOI) && !defined(TWODIMS) && !defined(ONEDIMS) /* will only be compiled in 3D case */
...
void initialize_and_create_first_tetra(tessellation *T)
{
  ...
  /*
     mpi_printf("side-length of enclosing tetraeder=%g\n", tetra_sidelength);
   */
...
//! get the line segments of a Voronoi cell
...
int insert_point(tessellation *T, int pp, int ttstart) /* returns a tetra that (currently) contains the point pp */
{
