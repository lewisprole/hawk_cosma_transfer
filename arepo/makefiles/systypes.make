...
ifeq ($(SYSTYPE),"Gertrude")
MPICHLIB =
CC       =  mpicc -std=c99 -Wno-format-security
FC       =  mpif90
OPTIMIZE =  -O3
GSL_INCL =  -I/usr/local/include/gsl
GSL_LIBS =  -L/usr/local/lib/ -lgsl -lgsl -lgslcblas -lm
FFTW_INCL=
FFTW_LIBS=
HDF5INCL =
HDF5LIB  =
HWLOC_INCL= -I/usr/include/
HWLOC_LIB = -L/usr/local/lib -lhwloc -Xlinker -R -Xlinker /usr/local/lib
LINKER   = $(FC)
ifeq (PRIMCHEM,$(findstring PRIMCHEM,$(CONFIGVARS)))
ODE_INCL =  -I/usr/local/include
ODE_LIB  =  -L/usr/local/lib -lsundials_cvode -lsundials_nvecserial
...
ifeq ($(SYSTYPE), "Cobra")
  ...
  # module load gcc/9 impi/2019.4 gsl fftw-mpi hdf5-serial
  CC       =  mpicc -std=c11 -Wall -Wextra -Wno-unused -Wno-format-overflow
  LINKER   =  mpicc -Xlinker -rpath=$(GSL_HOME)/lib:$(FFTW_HOME)/lib:$(HDF5_HOME)/lib
...
ifeq ($(SYSTYPE), "CobraOpenMPI")
  # HPC system Cobra at MPCDF, Garching:
  #   https://www.mpcdf.mpg.de/services/computing/cobra/
  # module load gsl fftw-mpi hdf5-serial
  # need to start job with /u/vrs/Libs/openmpi-4.0.1/bin/mpiexec 
  CC       =  //u/vrs/Libs/openmpi-4.0.1/bin/mpicc -std=c11 -Wall -Wextra -Wno-unused -Wno-format-overflow
...
ifeq ($(SYSTYPE), "draco")
  ...
  CC       =  mpiicc  -std=c11   -w3 -Wno-unused -diag-disable=424,981,1418,1572,1599,2259,2282,3280,11074,11076
  CPP      =  mpiicpc -std=c++11 -w3 -Wno-unused -diag-disable=424,981,1418,1572,1599,2259,2282,3280,11074,11076
  LINKER   =  mpiicc -Xlinker -rpath=$(GSL_HOME)/lib:$(FFTW_HOME)/lib:$(HDF5_HOME)/lib
  OPTIMIZE =  -g -O2 -march=native -ip -ipo
  GSL_INCL =  -I$(GSL_HOME)/include/
  GSL_LIBS =  -L$(GSL_HOME)/lib/
  FFTW_INCL=  -I$(FFTW_HOME)/include/
  FFTW_LIBS=  -L$(FFTW_HOME)/lib/
...
ifeq ($(SYSTYPE), "Freya")
  ...
  # at MPCDF, Garching:
  #   https://www.mpcdf.mpg.de/services/computing/linux/Astrophysics
  # module load intel impi fftw-mpi hdf5-serial gsl
  # disabled ICC compiler warnings/remarks:
  #   #424: extra ";" ignored
  #   #981: operands are evaluated in unspecified order
...
ifeq ($(SYSTYPE), "FreyaOpenMPI")
  ...
  # at MPCDF, Garching:
  #   https://www.mpcdf.mpg.de/services/computing/linux/Astrophysics
  # module load gcc impi fftw-mpi hdf5-serial gsl
...
ifeq ($(SYSTYPE), "SuperMuc-NG")
  ...
  HDF5LIB  = $(HDF5_CPP_LIB) -lhdf5 -lpthread
...
ifeq ($(SYSTYPE), "Ubuntu")
  ...
  CC        = mpicc  -std=c11   -Wall -Wextra -Wno-unused -Wno-format-overflow
  CPP       = mpic++ -std=c++11 -Wall -Wextra -Wno-unused -Wno-format-overflow
  FC        = mpif90
  OPTIMIZE  = -g -O2 -march=native
  ifeq (NUM_THREADS, $(findstring NUM_THREADS, $(CONFIGVARS)))
    OPTIMIZE += -fopenmp
